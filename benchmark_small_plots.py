...
def plot_avg_ops_per_thread(df, implementation_name, column='average_operations_per_thread'):
    ...
    df_exploded = explode_average_ops_per_thread(df, column=column)
    ax = sns.barplot(data=df_exploded, x='Thread', y=column)
    plt.title(f"Barplot of average operations per thread for {implementation_name} implementation")
