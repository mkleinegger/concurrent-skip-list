...
# @date 2025-01-13
#
# @brief All the necessary utilities for plotting the benchmark results.
...
def load_and_prepare_data(base_path="data"):
    ...
    for fpath in csv_files:
        ...
        implementation_name = parts[-3]  # e.g., 'sequential', 'fine_lock'
        ...
        if "average_operations_per_thread" in df.columns:
            ...
            df["average_operations_per_thread"] = df[
                "average_operations_per_thread"
            ].apply(lambda x: sanitize_average_ops(x))
...
def enrich_df(df):
    df_sequential_101080 = df[
        (df["implementation_name"] == "sequential") & (df["op_mix"] == "101080")
    ].copy()
    df_sequential_101080["range_type"] = "disjoint"
    df_sequential_404020 = df[
        (df["implementation_name"] == "sequential") & (df["op_mix"] == "404020")
    ].copy()
    df_sequential_404020["range_type"] = "disjoint"

    return pd.concat(
        [df, df_sequential_404020, df_sequential_101080], ignore_index=True
    )


def plot_throughput(df, op_mix, range_type, runtime, store=False, base_path="plots"):
    df_filtered = df.loc[
        (df["op_mix"] == op_mix)
        & (df["range_type"] == range_type)
        & (df["runtime_in_sec"] == runtime)
    ...
    plt.tight_layout()
    if store:
        os.makedirs(f"{base_path}/all_impl", exist_ok=True)
        plt.savefig(
...
def plot_total_vs_successful_operations(
    df,
    implementation_name,
    op_mix,
    range_type,
    ...
    palette = {"Inserts": "tab:blue", "Deletes": "tab:orange", "Contains": "tab:green"}
    linestyles = {"Total": "solid", "Successful": "dashed"}
    operations = {
    ...
    plt.tight_layout()
    if store:
        os.makedirs(f"{base_path}/{implementation_name}", exist_ok=True)
        plt.savefig(
...
def plot_total_vs_successful_operations_all_implementations(
    df, op, op_mix, range_type, runtime_in_sec, store=False, base_path="plots"
):
    filtered_df = df[
        (df["op_mix"] == op_mix)
        & (df["range_type"] == range_type)
    ...
    for implementation in implementation_names:
        impl_df = filtered_df[
            filtered_df["implementation_name"] == implementation
...
def plot_success_ratio_all_implementations(
    ...
    filtered_df.loc[:, "success_ratio"] = filtered_df[success_col] / filtered_df[
        total_col
    ].replace(0, pd.NA)
    ...
    plt.legend(
        title="Implementation", fontsize=12, title_fontsize=14, loc="lower right"
    )
    plt.tight_layout()
...
def plot_speedup_vs_sequential(
    df, op_mix, range_type, runtime, sequential_impl="sequential", store=False, base_path="plots"
):
    df_filtered = df.loc[(df["op_mix"] == op_mix) & (df["range_type"] == range_type) & (df["runtime_in_sec"] == runtime)].copy()
    df_filtered["throughput"] = df_filtered["total_operations"] / df_filtered["time"]
    df_seq = df_filtered[(df_filtered["implementation_name"] == sequential_impl) & (df_filtered["threads"] == 1)]
